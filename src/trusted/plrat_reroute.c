...
u64 plrat_reroute_get_proxy_rank(size_t id) {
...
FILE* plrat_reroute_get_proxy_file(size_t id) {
    return importfiles[plrat_reroute_get_proxy_rank(id)];
}
...
void plrat_reroute_init(const char* main_path, unsigned long solver_id, unsigned long num_solvers, unsigned long redistribution_strategy) {
    ...
    for (size_t i = 0; i < comm_size; i++) {
        char proof_path[512];
        if(redist_strat == 2) {
            snprintf(proof_path, 512, "%s/%lu/%lu.plrat_proxy", out_path, plrat_reroute_get_proxy_rank(i), local_id);
        } else {
...
void plrat_reroute_end() {
    ...
    for (size_t i = 0; i < comm_size; i++) {
        ...
        plrat_reroute_write_int(clauses[i]->size, current_out);
