...
void fill_buffer(struct plrat_reader* reader) {
    ...
    u64 bytes_read = UNLOCKED_IO(fread)(reader->read_buffer, read_size, 1, reader->buffered_file);
    if (MALLOB_UNLIKELY(bytes_read == 0)) {
        printf("ERROR\n");
    }
    reader->pos = reader->read_buffer;
    reader->end = reader->pos + bytes_read;
    reader->remaining_bytes -= bytes_read;
}

struct plrat_reader* plrat_reader_init(u64 buffer_size_bytes, FILE* file){
...
bool plrat_reader_check_bounds(u64 nb_bytes, struct plrat_reader* reader){
    ...
    if(nb_bytes > (u64)bytes_till_end){
...
void plrat_reader_skip_bytes(u64 nb_bytes, struct plrat_reader* reader){
    reader->pos += nb_bytes;
    long bytes_till_end = reader->end - reader->pos;
    if (MALLOB_UNLIKELY(bytes_till_end <= 0)){ // reader has reached the end of the buffer
        fill_buffer(reader);
